...
let classList: IterableList<Student> = {name: "Pepe", next: {name: "Juan", next: {name: "Luis", next: null}}};
 ...
 */
type RemoveZeroes<T extends any[]> = 
  T extends [infer Head, ...infer Tail] // Desestructurar el array en Head (primer elemento) y Tail (resto del array)
...
const createState = <T extends object>(initialState: T) => {
  ...
  return {
    setState: (partialState: Partial<T>): T => (state = { ...state, ...partialState }), //combina el estado actual con las nuevas propiedades proporcionadas.
  };
};
